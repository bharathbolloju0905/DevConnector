import React from 'react'
import { useState } from 'react'
import { useNavigate } from 'react-router-dom'
import toast from 'react-hot-toast'
import SigninHook from '../../hooks/Signin'
import { Link } from 'react-router-dom';
const Signin = () => {
  
    const { signin, loading } = SigninHook();
    const [input, setinput] = useState({
        email: "",
        password: ""
    });
    function handleChange(e) {
        setinput({
            ...input,
            [e.target.name]: e.target.value
        })
    }
  async function handleSubmit(e) {
        e.preventDefault();
        console.log(input);
       await signin(input);
    }

    return (
        <div className='h-screen w-full flex '>
            <div className='h-screen w-full lg:w-1/2 bg-[#e7effe] flex items-center justify-center'>
                <div className='flex flex-col items-start justify-center gap-[2rem] w-2/4'>
                    <div className='w-full'>
                        <h1 className='text-3xl font-bold'>DevConnector</h1>
                        <p className='text-sm text-gray-500'>Connect with developers around the world</p>
                    </div>
                    <div className='w-full '>
                        <h2 className='text-2xl font-bold'>Welcome Back !</h2>
                        <form className='w-full flex flex-col gap-4 mt-4' onSubmit={handleSubmit}>

                            <label htmlFor="Email" className='text-sm text-gray-600'>Email address</label>
                            <input type="email" name="email" id="email" className='w-full h-12 border border-gray-300 rounded-md px-4 bg-white' placeholder='Enter your email address' value={input.email} onChange={handleChange} required />

                            <label htmlFor="Password" className='text-sm text-gray-600'>Password</label>
                            <input type="password" name="password" id="password" className='w-full h-12 border border-gray-300 rounded-md px-4 bg-white' placeholder='Enter your password' value={input.password} onChange={handleChange} required />
                            <button type="submit" onSubmit={handleSubmit
                            } disabled={loading} className='w-full h-12 bg-[#4C4EE7] text-white rounded-md font-bold mt-5 cursor-pointer'>{loading?"Loading...":"Sign In"}</button>
                            <p className='text-sm text-gray-500 text-center'>Don't have an account? <Link to="/signup" className='text-[#4C4EE7] font-bold'>Sign Up</Link></p>
                        </form>
                    </div>

                </div>
            </div>
            <div className='h-screen lg:w-1/2 bg-[#4C4EE7] lg:flex items-center justify-center flex-col hidden '>
                <div className='flex flex-col items-center justify-center gap-[2rem] w-2/4'>
                    <div className='w-full rounded-lg overflow-hidden'>
                        <img src="./team.jpg" alt="signin image" className='object-contain w-fit' />
                    </div>
                    <div className='w-full text-white flex flex-col items-start justify-center gap-4'>
                        <h1 className='text-2xl font-bold'>Join our global developer community</h1>
                        <p className='text-md'> Connect with developers worldwide, share your projects, collaborate on code, and expand your professional network. Access resources, mentorship, and job opportunities.</p>
                    </div>

                    <div className='w-full flex  items-start justify-center gap-4 font-semibold'>
                       <button className='bg-white p-[10px] text-[#4C4EE7] rounded-md'><a href="/learn">Learn More</a></button>
                      <button className='border-[1px] border-white p-[10px]  rounded-md text-white '><a href="/learn">Watch Demo</a></button>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default Signin